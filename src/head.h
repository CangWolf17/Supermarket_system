...
public:
    ...
    static bool add(vector<Users> &users,
        Users &new_user); // 添加用户函数，执行安全检查并用于将指定的users类添加入数组，若用户已存在则返回false
...
public:
    ...
    static void add(vector<Goods> goods, Goods new_goods); // 商品添加函数
